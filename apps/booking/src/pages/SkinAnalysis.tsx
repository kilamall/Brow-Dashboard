...
export default function SkinAnalysisPage() {
  ...
  // Reset requestSubmitted when switching analysis types
  useEffect(() => {
          ...
          >
           ...
           <div className="max-w-4xl mx-auto mb-12">
             <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl border border-gray-200/50 overflow-hidden">
               <div className="p-8 lg:p-12">
                 <div className="text-center mb-8">
                   <h2 className="text-2xl font-bold text-gray-900 mb-4">Upload Your Products</h2>
                   <p className="text-gray-600">
                     Upload photos of your skincare products for personalized routine recommendations
                   </p>
                 </div>
                 ...
                 <button
                   onClick={handleSubmit}
                   disabled={loading || productUploads.length === 0}
                   className="w-full bg-gradient-to-r from-purple-600 to-violet-600 text-white py-4 rounded-2xl font-bold text-lg hover:from-purple-700 hover:to-violet-700 transition-all duration-300 shadow-2xl hover:shadow-3xl transform hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                 >
                   {loading ? (
                     <div className="flex items-center justify-center gap-3">
                       <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white"></div>
                   ...
                   <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-xl">
                     <div className="flex items-center gap-3">
                       ...
                       <div>
                         <h3 className="font-semibold text-green-800">Analysis Submitted!</h3>
                         <p className="text-green-700 text-sm">
                           Your product analysis is being processed. Results will appear below once complete.
                         </p>
                       </div>
                     </div>
                   </div>
                            ...
                            {analysis.analysis.recommendedServices && Array.isArray(analysis.analysis.recommendedServices) && analysis.analysis.recommendedServices.length > 0 && (
                              <div className="bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-200 rounded-2xl p-6 shadow-sm">
                                <div className="flex items-center justify-between mb-4">
                                  <h3 className="font-bold text-teal-800 text-lg flex items-center">
                                  ...
                                  <button
                                    ...
                                    className="bg-gradient-to-r from-teal-600 to-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:from-teal-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg"
                                  >
                                    Book All Services
                                  </button>
                                </div>
                                <div className="space-y-3">
                                  {analysis.analysis.recommendedServices.map((service: any, index: number) => (
                                    <div key={index} className="bg-white rounded-xl p-4 border border-teal-100 hover:border-teal-300 transition-colors">
                                      <div className="flex items-start justify-between mb-2">
                                        <h4 className="font-semibold text-teal-800 text-base">{service.serviceName}</h4>
                                      ...
                                      <p className="text-teal-700 text-sm mb-3">{service.reason}</p>
                                      {service.frequency && (
                                        <div className="text-xs text-teal-600 mb-3">
                                          <strong>Recommended Frequency:</strong> {service.frequency}
                                        </div>
                                      )}
                                      <button
                                        onClick={() => navigate('/book', { state: { preselectedService: service.serviceName } })}
                                        className="w-full bg-teal-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-teal-700 transition-colors flex items-center justify-center gap-2"
                                      >
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {user?.email && analysis.status === 'completed' && (
                  <div className="mt-8">
                    ...
                    {analysis.analysis?.recommendedServices && analysis.analysis.recommendedServices.length > 0 && (
                      <div className="mb-4 p-6 bg-gradient-to-r from-terracotta to-orange-600 rounded-2xl text-center shadow-2xl">
                        <h3 className="text-white font-bold text-xl mb-2">Ready to Transform Your Skin?</h3>
                        <p className="text-white/90 text-sm mb-4">Book your recommended services and start your journey to healthier, glowing skin</p>
                        <button
                          onClick={() => navigate('/book')}
                          className="bg-white text-terracotta px-8 py-4 rounded-xl font-bold text-lg hover:bg-gray-50 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center gap-3 mx-auto"
                        >
                    ...
                    {/* Secondary Action - Email Results */}
                    <div className="text-center">
                      <button
                      ...
                      >
                        ...
                        {emailStatus === 'idle' && '📧 Email Results to Myself'}
                      </button>
                      {emailStatus === 'success' && (
